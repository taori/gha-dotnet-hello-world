# [gha-dotnet-hello-world](https://www.github.com/taori/gha-dotnet-hello-world)

## Project state

[![GitHub issues](https://img.shields.io/github/issues/taori/gha-dotnet-hello-world)](https://github.com/taori/gha-dotnet-hello-world/issues)
[![.GitHub](https://github.com/taori/gha-dotnet-hello-world/actions/workflows/CI.yml/badge.svg)](https://github.com/taori/gha-dotnet-hello-world/actions/workflows/CI.yml)
[![docker release](https://github.com/taori/gha-dotnet-hello-world/actions/workflows/docker-relerase.yml/badge.svg?event=workflow_dispatch)](https://github.com/taori/gha-dotnet-hello-world/actions/workflows/docker-relerase.yml)

<!--CoverageStart-->
![Code Coverage](https://img.shields.io/badge/Code%20Coverage-40%25-success?style=flat)

Package | Line Rate | Branch Rate | Health
-------- | --------- | ----------- | ------
DotnetGithubActionHelloWorld | 40% | 50% | ✔
**Summary** | **40%** (20 / 50) | **50%** (5 / 10) | ✔

_Minimum allowed line rate is 1%_

[Coverage details](https://taori.github.io/gha-dotnet-hello-world)
<!--CoverageEnd-->

## Market place

[View](https://github.com/marketplace/actions/gha-dotnet-hello-world)

## Description

This template was generated using [Amusoft.Templates](https://www.github.com/taori/Amusoft.Templates)